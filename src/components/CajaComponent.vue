  ...
  <div class="flex no-wrap bg-grey-2" style="min-height: 100vh; overflow: hidden;">
    ...
    <div class="flex-1 column">
      ...
      <div class="col row no-wrap overflow-hidden">
        ...
        <div :class="ticketAbierto ? 'col-8' : 'col-12'" class="column h-full">
          <div class="col overflow-auto q-pa-lg">
            <div v-if="loading" class="row q-col-gutter-lg">
              <div v-for="skeleton in 8" :key="skeleton" class="col-xs-12 col-sm-6 col-md-4 col-lg-3">
                <q-card flat bordered class="bg-white" style="border-radius: 16px; border-color: #e1e3e4; min-height: 260px;">
                  <q-card-section>
          ...
          <div class="bg-white row items-center justify-between q-px-xl q-py-md" style="border-top: 1px solid #e1e3e4; z-index: 5;">
            <div class="row items-center q-gutter-xl">
              <div>
                ...
                <div class="text-h6 text-weight-bold text-primary" style="line-height: 1.2;">{{ totalProductos }}</div>
              </div>
              ...
              <div>
                ...
                <div class="text-h6 text-weight-bold text-orange-9" style="line-height: 1.2;">{{ totalComandasActivas }}</div>
              </div>
            </div>
            <div>
              <q-btn 
        ...
        <div v-if="ticketAbierto" class="col-4 bg-white column shadow-4" style="border-left: 1px solid #e1e3e4; z-index: 10; height: 100%;">
            ...
            <div v-for="(item, index) in itemsTicket" :key="index" class="bg-white q-pa-sm" style="border: 1px solid #e1e3e4; border-radius: 12px;">
              <div class="row justify-between items-start q-mb-sm">
                <div class="col">
                  ...
                  <div class="text-primary text-weight-bold">${{ (item.producto.precio_unitario * item.cantidad).toFixed(2) }}</div>
                </div>
...
const productosFiltrados = computed(() => {
  ...
  if (!term) return base;
  return base.filter(p => (p.nombre || '').toLowerCase().includes(term) || (p.descripcion || '').toLowerCase().includes(term));
});
...
const total = computed(() => {
  return itemsTicket.value.reduce((suma, item) => suma + (item.producto.precio_unitario * item.cantidad), 0);
});
