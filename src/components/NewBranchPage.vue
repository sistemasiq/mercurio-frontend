<script setup lang="ts">
import { ref } from 'vue'

const claveSucursal = ref('')
const nombreSucursal = ref('')
const direccionCompleta = ref('')
const telefonoContacto = ref('')
const correoElectronico = ref('')

const administradorSeleccionado = ref(null)

const listaAdministradores = ref(['Juan Pérez', 'María López', 'Carlos Gómez', 'Ana Martínez'])
const listaAdministradoresFiltrada = ref([...listaAdministradores.value])

function filtrarAdministradores(val: string, update: (fn: () => void) => void) {
  update(() => {
    if (val === '') {
      listaAdministradoresFiltrada.value = [...listaAdministradores.value]
    } else {
      const needle = val.toLowerCase()
      listaAdministradoresFiltrada.value = listaAdministradores.value.filter((g) =>
        g.toLowerCase().includes(needle),
      )
    }
  })
}

function crearNuevoAdministrador() {
  console.log('Crear nuevo administrador')
}
</script>

<template>
  <q-page padding class="bg-grey-1">
    <!-- Encabezado -->
    <div class="row items-center justify-between q-mb-lg">
      <div>
        <q-breadcrumbs class="text-grey-7 q-mb-sm" active-color="dark">
          <q-breadcrumbs-el label="Inicio" />
          <q-breadcrumbs-el label="Administración" />
          <q-breadcrumbs-el label="Sucursales" />
          <q-breadcrumbs-el label="Nueva Sucursal" class="text-weight-bold" />
        </q-breadcrumbs>

        <h1 class="text-h4 text-weight-bold q-my-none text-dark">Registro de Nueva Sucursal</h1>
        <p class="text-subtitle1 text-grey-7 q-mt-xs q-mb-none">
          Configura los parámetros operativos de la nueva locación.
        </p>
      </div>

      <div class="row q-gutter-sm">
        <q-btn outline color="dark" label="Cancelar" class="btn-cancelar" />
        <q-btn color="primary" icon="save" label="Guardar Sucursal" class="btn-guardar" />
      </div>
    </div>

    <!-- Contenido principal -->
    <div class="row q-col-gutter-md">
      <!-- Columna izquierda (formulario) -->
      <div class="col-12 col-md-8">
        <!-- Card: Información General -->
        <q-card class="q-mb-lg form-card" flat bordered>
          <q-card-section class="card-header-section">
            <div class="text-h6 text-weight-bold flex items-center">
              <q-icon name="info" size="sm" color="primary" class="q-mr-sm" />
              Información General
            </div>
          </q-card-section>

          <q-separator />

          <q-card-section class="q-pt-lg">
            <div class="row q-col-gutter-md">
              <div class="col-12 col-md-6">
                <div class="field-label">Clave de Sucursal</div>
                <q-input
                  v-model="claveSucursal"
                  outlined
                  dense
                  placeholder="ej. SUC-LP-01"
                  class="field-input"
                />
              </div>
              <div class="col-12 col-md-6">
                <div class="field-label">Nombre de la Sucursal</div>
                <q-input
                  v-model="nombreSucursal"
                  outlined
                  dense
                  placeholder="ej. Plaza Colibrí"
                  class="field-input"
                />
              </div>
              <div class="col-12">
                <div class="field-label">Dirección Completa</div>
                <q-input
                  v-model="direccionCompleta"
                  outlined
                  type="textarea"
                  rows="3"
                  placeholder="Calle, Número, Colonia, Ciudad, Estado"
                  class="field-input"
                >
                  <template #prepend>
                    <q-icon name="place" color="grey-6" />
                  </template>
                </q-input>
              </div>
              <div class="col-12 col-md-6">
                <div class="field-label">Teléfono de Contacto</div>
                <q-input
                  v-model="telefonoContacto"
                  outlined
                  dense
                  placeholder="+52 000 000 0000"
                  class="field-input"
                >
                  <template #prepend>
                    <q-icon name="phone" color="grey-6" />
                  </template>
                </q-input>
              </div>
              <div class="col-12 col-md-6">
                <div class="field-label">Correo Electrónico</div>
                <q-input
                  v-model="correoElectronico"
                  outlined
                  dense
                  placeholder="sucursal@playground.os"
                  class="field-input"
                >
                  <template #prepend>
                    <q-icon name="mail" color="grey-6" />
                  </template>
                </q-input>
              </div>
            </div>
          </q-card-section>
        </q-card>

        <!-- Card: Gestión y Responsabilidades -->
        <q-card class="q-mb-lg form-card" flat bordered>
          <q-card-section class="card-header-section">
            <div class="text-h6 text-weight-bold flex items-center">
              <q-icon name="group" size="sm" color="primary" class="q-mr-sm" />
              Gestión y Responsabilidades
            </div>
          </q-card-section>

          <q-separator />

          <q-card-section class="q-pt-lg">
            <div class="field-label">Administrador Responsable</div>
            <div class="row q-col-gutter-md items-center">
              <div class="col-12 col-md-8">
                <q-select
                  v-model="administradorSeleccionado"
                  outlined
                  dense
                  use-input
                  input-debounce="0"
                  placeholder="Buscar por nombre o ID de usuario..."
                  :options="listaAdministradoresFiltrada"
                  clearable
                  class="field-input"
                  @filter="filtrarAdministradores"
                >
                  <template #prepend>
                    <q-icon name="search" color="grey-6" />
                  </template>
                  <template #no-option>
                    <q-item>
                      <q-item-section class="text-grey">Sin resultados</q-item-section>
                    </q-item>
                  </template>
                </q-select>
              </div>
              <div class="col-12 col-md-4">
                <q-btn
                  outline
                  color="primary"
                  icon="person_add"
                  label="+ Crear Nuevo Administrador"
                  class="full-width btn-nuevo-administrador"
                  @click="crearNuevoAdministrador"
                />
              </div>
            </div>
          </q-card-section>
        </q-card>
      </div>

      <!-- Columna derecha (tip) -->
      <div class="col-12 col-md-4">
        <q-card class="tip-card" flat>
          <q-card-section>
            <div class="flex items-center q-mb-sm">
              <q-icon name="lightbulb" color="primary" size="sm" class="q-mr-sm" />
              <span class="text-weight-bold text-primary">Tip de Administrador</span>
            </div>
            <p class="text-body2 text-grey-8 q-mb-none">
              Asegúrate de que la Clave de Sucursal siga el formato estandarizado
              <strong>(SUC-XX-00)</strong> para facilitar el filtrado en reportes globales.
            </p>
          </q-card-section>
        </q-card>
      </div>
    </div>
  </q-page>
</template>

<style scoped>
/* ── Cards ── */
.form-card {
  border-radius: 12px;
  border-color: #e0e0e0;
}

.card-header-section {
  padding: 20px 24px 16px;
}

/* ── Labels sobre los inputs ── */
.field-label {
  font-size: 13px;
  font-weight: 500;
  color: #555;
  margin-bottom: 6px;
}

/* ── Inputs: esquinas levemente redondeadas, sin label flotante ── */
.field-input :deep(.q-field__control) {
  border-radius: 8px;
  background: #ffffff;
}

.field-input :deep(.q-field__label) {
  display: none;
}

/* ── Botón Cancelar ── */
.btn-cancelar {
  border-radius: 8px;
  text-transform: none;
  font-weight: 500;
  padding: 8px 20px;
}

/* ── Botón Guardar Sucursal ── */
.btn-guardar {
  border-radius: 8px;
  text-transform: none;
  font-weight: 500;
  padding: 8px 20px;
}

/* ── Botón Crear Nuevo Administrador ── */
.btn-nuevo-administrador {
  border-radius: 8px;
  text-transform: none;
  font-weight: 500;
  height: 40px;
}

/* ── Tip de Administrador ── */
.tip-card {
  border-radius: 12px;
  background-color: #e8f0fe;
  border: 1px solid #c5d8fb;
}
</style>
