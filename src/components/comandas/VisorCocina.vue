...
const totalPendientes = computed(() => comandas.value.filter(comanda => comanda.estado_actual === 'P').length)
const totalEnProceso = computed(() => comandas.value.filter(comanda => comanda.estado_actual === 'E').length)
const totalListos = computed(() => comandas.value.filter(comanda => comanda.estado_actual === 'L').length)

const obtenerDetallesComanda = (comanda: Comanda) => {
  return comanda.detalles_comanda ?? (comanda as Comanda & { detalles?: Comanda['detalles_comanda'] }).detalles ?? []
}
...
<template>
  <div class="q-pa-xl bg-grey-1">
    ...
    <div v-else class="row q-col-gutter-lg">
      ...
      >
        <q-card class="kds-card flat bordered" :class="{
          ...
          <div class="full-width q-pa-md row column justify-between">
            <div>
              <div class="row justify-between items-start q-mb-sm">
                <div>
                  <span class="text-caption text-weight-bolder text-grey-6 block uppercase tracking-wider">
                    {{ comanda.estado_actual === 'P' ? 'PENDIENTE' : comanda.estado_actual === 'E' ? 'EN PREPARACIÓN' : 'LISTO PARA ENTREGA' }}
                  </span>
              ...
              <div class="q-mt-md order-items-container">
                <div v-for="detalle in obtenerDetallesComanda(comanda)" :key="detalle.id" class="product-row q-py-sm">
                  <div class="row items-start no-wrap q-gutter-sm">
                    <q-avatar size="30px" color="grey-2" text-color="grey-8" class="text-weight-bold">
                      {{ detalle.cantidad }}
                    </q-avatar>
                    ...
                    <div class="full-width">
                      ...
                      <div v-if="obtenerNotasDetalle(detalle)" class="q-mt-xs text-caption text-red">
