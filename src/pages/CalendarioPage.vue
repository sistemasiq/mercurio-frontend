    ...
    <div style="max-width: 1400px; margin: 0 auto;">
      ...
      <div class="row items-center q-mb-lg">
        <div>
          ...
          <div class="text-body2" style="color: var(--text-secondary);">Vista mensual de eventos y reservaciones.</div>
        </div>
        <q-space />
        <q-btn flat no-caps label="Hoy" color="primary" class="q-mr-sm" style="border-radius: 8px; font-weight: 600;" @click="irAHoy" />
        <q-btn flat dense round icon="chevron_left" color="grey-7" @click="prevMonth" />
        <span class="q-mx-sm" style="font-size: 1rem; font-weight: 700; color: var(--text-primary); min-width: 160px; text-align: center; text-transform: capitalize;">
          {{ monthLabel }}
        </span>
      ...
      <div class="row q-col-gutter-lg">
          ...
          <div class="panel-card" style="overflow: hidden;">
            ...
            <div v-else class="cal-grid">
              ...
              >
                ...
                <!-- Chips de eventos -->
                <div v-for="ev in day.events.slice(0, 2)" :key="ev.id" class="cal-chip" :class="`cal-chip--${ev.estado}`">
        ...
        <div class="col-12 col-md-4 col-lg-3">
          ...
          <div class="panel-card q-mb-md">
            ...
            <div v-else-if="eventosDelDia.length === 0" class="q-pa-md text-caption text-grey text-center q-py-lg">
...
onMounted(() => { if (!store.reservaciones.length) store.cargar() })
...
const prevMonth = () => { cursor.value = new Date(curYear.value, curMonth.value - 1, 1) }
const nextMonth = () => { cursor.value = new Date(curYear.value, curMonth.value + 1, 1) }
const irAHoy   = () => {
...
const eventos = computed((): EvResumen[] =>
  store.reservaciones
    ...
    .map(r => ({
      id: r.id,
      nombre: r.nombre_festejado || `${r.nombre_cliente}${r.apellidos_cliente ? ' ' + r.apellidos_cliente : ''}`.trim(),
      estado: r.estado,
...
const calendarDays = computed((): CalDay[] => {
  ...
  for (let i = firstWeekday - 1; i >= 0; i--) {
    ...
    days.push({ day: d, isoDate: isoDate(new Date(prevYear, prevMonth, d)), isToday: false, isOtherMonth: true, events: [] })
  }
  ...
  for (let d = 1; d <= next; d++) {
    ...
    days.push({ day: d, isoDate: isoDate(new Date(nextYear, nextMonth, d)), isToday: false, isOtherMonth: true, events: [] })
  }
...
const statConfirmados = computed(() => reservacionesMes.value.filter(r => r.estado === 'confirmada' || r.estado === 'completada').length)
const statPendientes  = computed(() => reservacionesMes.value.filter(r => r.estado === 'pendiente').length)
...
.cal-stat-card {
  ...
  &__icon {
    ...
    &--blue   { background: #e8eaf6; color: #3949ab; }
...
.cal-chip {
  ...
  &--confirmada { background: #e8eaf6; color: #283593; }
  &--en_curso   { background: #f3e5f5; color: #6a1b9a; }
  &--completada { background: #e8f5e9; color: #1b5e20; }
  &--cancelada  { background: #f5f5f5; color: #757575; }
  &--more       { background: transparent; color: #90a4ae; font-style: italic; }
}
...
.ev-dot--pendiente  { background: #e65100; }
.ev-dot--confirmada { background: #283593; }
.ev-dot--en_curso   { background: #6a1b9a; }
.ev-dot--completada { background: #1b5e20; }
.ev-dot--cancelada  { background: #9e9e9e; }
