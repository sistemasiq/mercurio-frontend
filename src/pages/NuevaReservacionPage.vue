    ...
    <div style="width: 100%; max-width: 1800px; margin: 0 auto;">
      ...
      <div class="row q-col-gutter-xl">
        ...
        <div class="col-12 col-md-8 col-lg-9">
          <q-stepper v-model="step" ref="stepper" color="primary" flat class="bg-transparent" style="padding: 0;">
              ...
              <div class="form-section">
                ...
                <div class="row q-gutter-md">
                  ...
                  <div class="col">
                    ...
                    <q-input v-model="form.telefono" dense outlined placeholder="+52 000 000 0000" />
                  </div>
                </div>

                <div class="q-mt-md">
                  <div class="field-label">CORREO ELECTRÓNICO</div>
                  <q-input v-model="form.email" dense outlined placeholder="cliente@ejemplo.com" type="email" />
                </div>
            ...
            <q-step :name="2" title="Paquetes y Extras" icon="celebration" :done="step > 2">
              ...
              <div class="form-section">
                ...
                <div v-if="paquetesStore.loading" class="q-pa-md text-grey">Cargando paquetes...</div>
                <div v-else-if="!paquetesStore.activos.length" class="q-pa-md text-grey">No hay paquetes disponibles</div>
                <div v-else class="row q-gutter-md">
                  ...
                  >
                    <div v-if="paquetesStore.activos.indexOf(pkg) === 0" class="package-card__badge">Recomendado</div>
                    <div class="package-card__name">{{ pkg.nombre }}</div>
                    <div class="package-card__capacity">Hasta {{ pkg.personas_incluidas }} personas</div>
                    <div class="package-card__price">{{ fmt(parseFloat(pkg.precio_base)) }}</div>
                    <ul class="package-card__features">
                      <li v-if="pkg.descripcion"><q-icon name="check_circle" />{{ pkg.descripcion }}</li>
                      <li><q-icon name="check_circle" />{{ pkg.duracion_minutos }} min. de duración</li>
                      <li v-if="parseFloat(pkg.precio_persona_extra) > 0">
                        <q-icon name="check_circle" />+{{ fmt(parseFloat(pkg.precio_persona_extra)) }} por persona extra
                      </li>
                    </ul>
                    <q-btn
                      unelevated
                      ...
                      class="full-width" style="border-radius: 8px; font-weight: 700;" no-caps
                    />
                  </div>
                </div>
              </div>
              ...
              <div class="form-section">
                ...
                <div v-else-if="!extrasStore.activos.length" class="q-pa-md text-grey">No hay extras disponibles</div>
                <div v-else class="row q-gutter-md">
                  <div
                  ...
                  >
                    ...
                    <div class="service-card__body">
                      ...
                      <q-btn
                        ...
                        :style="selectedExtraIds.includes(svc.id)
              ...
              <q-stepper-navigation class="q-mt-lg flex items-center">
                <q-btn @click="step = 3" color="primary" label="Continuar a Pago" class="q-px-md" unelevated style="border-radius: 8px; font-weight: 600;" no-caps />
            ...
            <q-step :name="3" title="Anticipo" icon="payments" :done="step > 3">
              ...
              <div class="text-h6 q-mb-md" style="font-weight: 800; color: var(--text-primary);">Registra el anticipo</div>
              <p class="text-body2 text-grey-8 q-mb-lg">Es necesario registrar el anticipo para poder confirmar la reservación.</p>

              <!-- Banner de éxito -->
              <div
                v-if="pagoRegistrado"
                class="q-pa-md q-mb-lg rounded-borders"
                style="background: #e8f5e9; border: 1px solid #a5d6a7; display: flex; align-items: center; gap: 12px;"
              >
                <q-icon name="check_circle" color="positive" size="28px" />
                <div>
                  <div style="font-weight: 700; color: #2e7d32;">Anticipo registrado correctamente</div>
              ...
              <template v-if="!pagoRegistrado">
                <div class="q-pa-md bg-white rounded-borders shadow-1 q-mb-lg" style="border: 1px solid var(--border-color);">
                  ...
                  <div class="row justify-between">
                    ...
                    <span style="font-weight: 700; color: var(--q-primary);">{{ advanceAmount }}</span>
                  </div>
                </div>
                ...
                <div class="q-mb-md">
                  ...
                  <q-input
                    ...
                    dense outlined type="number" prefix="$"
            ...
            <q-step :name="4" title="Confirmación" icon="check_circle">
              ...
              <div class="text-h6 q-mb-md" style="font-weight: 800; color: var(--text-primary);">Resumen de la Reservación</div>
              <p class="text-body2 text-grey-8 q-mb-lg">Verifica todos los datos antes de confirmar.</p>

              <!-- Bloque: Cliente -->
              <div class="q-pa-md bg-white rounded-borders shadow-1 q-mb-md" style="border: 1px solid var(--border-color);">
                <div class="resumen-section-title">Cliente</div>
                <div class="resumen-row"><span>Nombre</span><span>{{ form.nombre || '—' }}</span></div>
                <div class="resumen-row"><span>Teléfono</span><span>{{ form.telefono || '—' }}</span></div>
                <div class="resumen-row"><span>Email</span><span>{{ form.email || '—' }}</span></div>
              </div>

              <!-- Bloque: Evento -->
              <div class="q-pa-md bg-white rounded-borders shadow-1 q-mb-md" style="border: 1px solid var(--border-color);">
                <div class="resumen-section-title">Evento</div>
                <div class="resumen-row"><span>Tipo</span><span>{{ tipoEventoNombre }}</span></div>
                <div class="resumen-row"><span>Fecha</span><span>{{ selectedDateLabel }}</span></div>
                <div class="resumen-row"><span>Horario</span><span>{{ timeSlotLabel }}</span></div>
                <div class="resumen-row"><span>Personas</span><span>{{ form.ninos }}</span></div>
              </div>

              <!-- Bloque: Paquete y Extras -->
              <div class="q-pa-md bg-white rounded-borders shadow-1 q-mb-md" style="border: 1px solid var(--border-color);">
                <div class="resumen-section-title">Paquete y Extras</div>
                <div class="resumen-row"><span>Paquete</span><span>{{ selectedPackageName || '—' }}</span></div>
                <div class="resumen-row"><span>Precio base</span><span>{{ packagePrice }}</span></div>
                <template v-if="extrasSeleccionados.length">
                  <div class="resumen-row" v-for="e in extrasSeleccionados" :key="e.id">
                    <span>{{ e.nombre }}</span><span>{{ fmt(parseFloat(e.precio)) }}</span>
                  </div>
                </template>
                <div v-else class="resumen-row"><span class="text-grey-6">Extras</span><span class="text-grey-6">Ninguno</span></div>
              </div>

              <!-- Bloque: Pago -->
              <div class="q-pa-md bg-white rounded-borders shadow-1 q-mb-lg" style="border: 1px solid var(--border-color);">
                <div class="resumen-section-title">Pago</div>
                <div class="resumen-row"><span>Total</span><span style="font-weight: 700;">{{ totalAmount }}</span></div>
                <div class="resumen-row">
                  <span>Anticipo pagado</span>
                  <span style="color: #2e7d32; font-weight: 600;">{{ fmt(montoPagado) }}</span>
                </div>
                <div class="resumen-row"><span>Método</span><span>{{ metodoPagoNombre }}</span></div>
                <div class="resumen-row">
              ...
              <div class="q-pa-md bg-white rounded-borders shadow-1 q-mb-lg" style="border: 1px solid var(--border-color);">
                <q-checkbox v-model="form.termsAccepted" dense style="align-items: flex-start;">
                  <span style="font-size: 0.85rem; line-height: 1.4; color: #546e7a;">
                    He revisado los datos del cliente y la disponibilidad de fecha con el reglamento de cancelación vigente.
                  </span>
                </q-checkbox>
              </div>
              ...
              <q-stepper-navigation class="q-mt-lg flex items-center">
                ...
                />
                <q-btn flat @click="step = 3" color="primary" label="Atrás" class="q-ml-sm" no-caps />
              </q-stepper-navigation>
            </q-step>
          ...
          <div class="payment-card sticky-payment">
            ...
            <template v-else>
              ...
              <template v-if="pagoRegistrado">
                ...
                <div class="payment-card__row" style="border-top: 2px solid #e2e8f0; margin-top: 4px; padding-top: 8px;">
                  <span style="font-weight: 700; color: var(--text-primary);">Saldo pendiente</span>
                  <span style="font-weight: 800; font-size: 1rem; color: var(--q-primary);">{{ fmt(totalNum - montoPagado) }}</span>
                </div>
              </template>
              ...
              <template v-else>
                <div class="payment-card__advance">
                  ...
                  <div class="advance-note">
                    El resto ({{ selectedPkg ? remainingAmount : '—' }}) se liquida el día del evento
                  </div>
                </div>
              </template>
            </template>
...
const selectedDateLabel = computed(() => {
  ...
  return new Date(currentYear.value, currentMonth.value, form.value.selectedDay)
...
const selectPackage = (id: string) => { form.value.selectedPackage = id }
...
const confirmarReservacion = async () => {
  ...
  if (!fecha || !form.value.tipoEvento || !form.value.selectedPackage) {
    $q.notify({ type: 'warning', message: 'Completa todos los datos requeridos', position: 'top-right' })
    return
  }
  ...
  try {
    ...
    $q.notify({ type: 'positive', message: 'Reservación confirmada exitosamente', position: 'top-right' })
