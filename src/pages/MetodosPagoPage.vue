  ...
  <q-page class="page-content q-pa-md q-pa-lg-xl">
    <div style="max-width: 900px; margin: 0 auto;">
      ...
      <div class="row items-center q-mb-lg">
        <div>
          <div class="text-h5 text-weight-bold" style="color: var(--text-primary);">Métodos de Pago</div>
          <div class="text-body2" style="color: var(--text-secondary);">Gestiona los métodos de pago disponibles para reservaciones.</div>
        </div>
        <q-space />
        <q-btn color="primary" icon="add" label="Nuevo Método" unelevated no-caps style="border-radius: 8px; font-weight: 600;" @click="abrirCrear" />
      </div>

      <!-- Error -->
      <q-banner v-if="store.error" dense rounded class="bg-red-1 text-red-8 q-mb-md" style="border-radius: 10px;">
    ...
    <q-dialog v-model="dialogOpen" persistent>
      <q-card style="min-width: 420px; border-radius: 12px;">
        <q-card-section class="q-pb-sm">
          <div class="text-h6 text-weight-bold">{{ editando ? 'Editar Método de Pago' : 'Nuevo Método de Pago' }}</div>
        </q-card-section>
        ...
        <q-card-actions align="right" class="q-pa-md q-pt-sm">
          <q-btn flat no-caps label="Cancelar" @click="cerrarDialog" color="grey-7" />
          <q-btn
    ...
    <q-dialog v-model="dialogEliminar">
      <q-card style="min-width: 360px; border-radius: 12px;">
        ...
        <q-card-actions align="right" class="q-pa-md q-pt-xs">
          ...
          <q-btn
            unelevated no-caps color="negative" label="Eliminar"
            style="border-radius: 8px; font-weight: 600;"
            :loading="eliminando"
...
const guardar = async () => {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: 'Ocurrió un error. Intenta de nuevo.', position: 'top-right' })
  } finally {
...
const ejecutarEliminar = async () => {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: 'No se pudo eliminar. Intenta de nuevo.', position: 'top-right' })
  } finally {
