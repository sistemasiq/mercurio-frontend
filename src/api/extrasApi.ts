...
export const extrasApi = {
  ...
  obtener: (extra_id: string) => apiClient.get<Extras>(`/extras/${extra_id}`).then((r) => r.data),

  crear: (body: ExtrasCreate) => apiClient.post<Extras>('/extras', body).then((r) => r.data),

  actualizar: (extra_id: string, body: ExtrasUpdate) =>
    apiClient.patch<Extras>(`/extras/${extra_id}`, body).then((r) => r.data),

  eliminar: (extra_id: string) => apiClient.delete(`/extras/${extra_id}`).then((r) => r.data),
}
