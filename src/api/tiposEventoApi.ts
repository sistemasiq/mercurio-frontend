...
export const tiposEventoApi = {
  listar: () => apiClient.get<Tipos_evento[]>('/tipos-evento').then((r) => r.data),

  obtener: (id: string) => apiClient.get<Tipos_evento>(`/tipos-evento/${id}`).then((r) => r.data),
  ...
  eliminar: (id: string) => apiClient.delete(`/tipos-evento/${id}`).then((r) => r.data),
}
